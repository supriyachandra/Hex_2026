...
export function ActiveAdmissions() {
    ...
    const api = "http://localhost:8080/api/admission/get-all-active";
    ...
    const handleDischarge = async (id) => {
        try {
            await axios.put(`http://localhost:8080/api/admission/discharge/${id}`, {}, {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("token")
                }
            });
                    ...
                    {data.map((a) => (
                        <tr key={a.reference_id}>
                            ...
                            <td className="text-end">
                                ...
                                {a.admission_status === "DISCHARGE_REQUESTED" && (
                                    <button
                                        className="btn btn-danger btn-sm"
                                        onClick={() => handleDischarge(a.reference_id)}
                                    >
                                        Discharge
                                    </button>
                                )}
